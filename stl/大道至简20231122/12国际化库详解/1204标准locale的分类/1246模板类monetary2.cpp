...
int main()
{
    ...
    const moneypunct<char, true> &mpunct4 = use_facet<moneypunct<char, true>>(loc);
    cout << loc.name() << " domestic decimal point " << mpunct4.decimal_point() << endl;
    for (unsigned int i = 0; i < mpunct.grouping().length(); i++)
    {
        cout << loc.name() << " international grouping:\n the " << i << "th group to the left of the ra￾dix character "
    ...
    cout << loc.name() << " domestic frac_digits\n to the right"
         << " of the radix character: " << mpunct5.frac_digits() << endl
         << endl;
    const moneypunct<char, true> &mpunct6 = use_facet<moneypunct<char, true>>(loc);
    cout << loc.name() << " international negative number format: " << mpunct6.neg_format().field[0]
